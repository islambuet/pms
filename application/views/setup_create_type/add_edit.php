    ...
    <div class="row widget">
        ...
        <div style="" class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <select id="crop_id" name="type[crop_id]" class="form-control" tabindex="-1">
                    <option value=""><?php echo $this->lang->line('SELECT');?></option>
                    <?php
                    foreach($crops as $crop)
                    {?>
                        <option value="<?php echo $crop['id']?>" <?php if($crop['id']==$type['crop_id']){ echo "selected";}?>><?php echo $crop['crop_name'];?></option>
                    <?php
                    }
                    ?>
                </select>
        ...
        <div style="<?php if(!($type['classification_id']>0)){echo 'display:none';} ?>" class="row show-grid" id="classification_id_container">
            ...
            <div class="col-sm-4 col-xs-8">
                <select id="classification_id" name="type[classification_id]" class="form-control" tabindex="-1">
                    ...
                    {?>
                        <option value="<?php echo $classification['id']?>" <?php if($classification['id']==$type['classification_id']){ echo "selected";}?>><?php echo $classification['classification_name'];?></option>
                    <?php
                    }
                    ?>
                </select>
        ...
        <div class="row show-grid">
            ...
            <div class="col-sm-4 col-xs-8">
                <input type="text" name="type[type_name]" id="crop_name" class="form-control validate[required]" value="<?php echo $type['type_name'];?>"/>
            </div>
        </div>
