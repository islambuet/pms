...
class Setup_cc_classification extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Setup_cc_classification');
        $this->controller_url='setup_cc_classification';
    ...
    private function system_list()
    {
        ...
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("setup_cc_classification/list",$data,true));
            if($this->message)
            {
    ...
    {
        $this->db->from($this->config->item('table_cc_classifications').' classifications');
    ...
    private function system_add()
    {
        if(isset($this->permissions['action1'])&&($this->permissions['action1']==1))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("setup_cc_classification/add_edit",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['action2'])&&($this->permissions['action2']==1))
        {
            ...
            $data['classification']=Query_helper::get_info($this->config->item('table_cc_classifications'),'*',array('id ='.$classification_id),1);
            $data['crops']=Query_helper::get_info($this->config->item('table_cc_crops'),array('id value','name text'),array('status ="'.$this->config->item('system_status_active').'"'),0,0,array('ordering ASC'));
            $data['title']="Edit Classification (".$data['classification']['name'].')';
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("setup_cc_classification/add_edit",$data,true));
            if($this->message)
            {
    ...
    private function system_save()
    {
        ...
        else
        {
            ...
            if($id>0)
            {
                ...
                Query_helper::update($this->config->item('table_cc_classifications'),$data,array("id = ".$id));
            }
            else
            {
                $data['user_created'] = $user->user_id;
                $data['date_created'] = $time;
                Query_helper::add($this->config->item('table_cc_classifications'),$data);
            }
    ...
    private function check_validation()
    {
        ...
        $this->form_validation->set_rules('classification[name]',$this->lang->line('LABEL_CLASSIFICATION_NAME'),'required');
