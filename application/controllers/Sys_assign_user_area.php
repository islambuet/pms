...
class Sys_assign_user_area extends Root_Controller
{
    ...
    private function get_items()
    {
        ...
        $this->db->join($this->config->item('table_location_unions').' union','union.id = aa.union_id','LEFT');
        $this->db->join($this->config->item('table_location_upazilas').' u','u.id = aa.upazila_id','LEFT');
        $this->db->join($this->config->item('table_location_districts').' d','d.id = aa.district_id','LEFT');
        $this->db->join($this->config->item('table_location_territories').' t','t.id = aa.territory_id','LEFT');
        $this->db->join($this->config->item('table_location_zones').' zone','zone.id = aa.zone_id','LEFT');
        ...
        foreach($items as &$item)
        {
            if(isset($areas[$item['id']]))
            {
                ...
                if($areas[$item['id']]['upazila_name'])
                {
                    $item['upazila_name']=$areas[$item['id']]['upazila_name'];
                }
                else
                {
                    $item['upazila_name']='ALL';
                }
            ...
            else
            {
                ...
                $item['upazila_name']='Not Assigned';
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['edit'])&&($this->permissions['edit']==1))
        {
            ...
            if($data['assigned_area'])
            {
                ...
                {
                    $this->db->join($this->config->item('table_location_zones').' zone','zone.id = aa.zone_id','INNER');
                }
                if($data['assigned_area']['territory_id']>0)
                {
                    $this->db->join($this->config->item('table_location_territories').' t','t.zone_id = zone.id','INNER');
                    $this->db->where('t.id',$data['assigned_area']['territory_id']);
                }
                if($data['assigned_area']['district_id']>0)
                {
                    $this->db->join($this->config->item('table_location_districts').' d','d.territory_id = t.id','INNER');
                    $this->db->where('d.id',$data['assigned_area']['district_id']);
                }
                if($data['assigned_area']['upazila_id']>0)
                {
                    $this->db->join($this->config->item('table_location_upazilas').' u','u.district_id = d.id','INNER');
                    $this->db->where('u.id',$data['assigned_area']['upazila_id']);
                }
                if($data['assigned_area']['union_id']>0)
                {
                    $this->db->join($this->config->item('table_location_unions').' union','union.upazila_id = u.id','INNER');
    ...
    private function check_validation()
    {
        ...
        if($data['union_id']>0)
        {
            $this->form_validation->set_rules('area[upazila_id]',$this->lang->line('LABEL_UPAZILA_NAME'),'required|is_natural_no_zero');
        }
        if($data['upazila_id']>0)
        {
